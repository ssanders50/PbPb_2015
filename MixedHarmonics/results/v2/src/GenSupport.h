...
int ep2a, int ep2b, int ep2c,int ep3a, int ep3b, int ep3c, int mintrk, int maxtrk,int subevt);
  ...
  TH1D * getSPIntnum(double minpt, double maxpt);
...
int ep3a, int ep3b, int ep3c, int mintrk, int maxtrk, int subevt) {
  ...
  spvnnum             = (TH2D *) rescor->Clone(Form("spvnnum%s",utag.data()));
  spvnnum->SetXTitle(pttitle.data());
  ...
  spInt = new TH1D(Form("spInt%s",utag.data()),"spInt",etabins->GetNbinsX(),etabins->GetXaxis()->GetXbins()->GetArray());
  spInt->SetXTitle(etatitle.data());
  spIntnum = new TH1D(Form("spIntnum%s",utag.data()),"spIntnum",etabins->GetNbinsX(),etabins->GetXaxis()->GetXbins()->GetArray());
  spIntnum->SetXTitle(etatitle.data());
  spepintratio = new TH1D(Form("spepintratio%s",utag.data()),"spepintratio",etabins->GetNbinsX(),etabins->GetXaxis()->GetXbins()->GetArray());
  spepintratio->SetXTitle(etatitle.data());
  q.resize(etabins->GetNbinsX(), vector<Qvec *>(ptbins->GetNbinsX()));
  qav = new Qvec(0,0,Form("qav_%s",utag.data()));
  for(int i = 0; i<ptbins->GetNbinsX(); i++) {
    for(int j = 0; j<etabins->GetNbinsX(); j++) {
      string name = Form("q_%d_%d_%s",i,j,utag.data());
      q.at(j).at(i) = new Qvec(i,j,name);
    }
  }

  for(int k = 0; k<ERRCNT; k++) {
    string avname = "qaverr_"+to_string(k)+utag.data();
    qaverr[k] = new Qvec(0,0,avname);
    qerr[k].resize(etabins->GetNbinsX(), vector<Qvec *>(ptbins->GetNbinsX()));
    for(int i = 0; i<ptbins->GetNbinsX(); i++) {
      for(int j = 0; j<etabins->GetNbinsX(); j++) {
  	string name = Form("qerr_%d_%d_%d_%s",k,i,j,utag.data());
...
TH2D * GenSupport::getSPVNnum(){

  for(int i = 0; i<ptbins->GetNbinsX(); i++) {
    for(int j = 0; j<etabins->GetNbinsX(); j++) {
      if(!std::isnormal(q.at(j).at(i)->getSPVNnum())) {
	spvnnum->SetBinContent(i+1,j+1,0);
	spvnnum->SetBinError(i+1,j+1,0);
	continue;
      };
      spvnnum->SetBinContent(i+1,j+1, q.at(j).at(i)->getSPVNnum());
	...
	if(ncnt > 1) {
      ...
      if(std::isnormal(err)) {
	spvnnum->SetBinError(i+1,j+1,err);
      } else {
	spvnnum->SetBinError(i+1,j+1,0.);
      }
    }
...
TH1D * GenSupport::getEPInt(double minpt, double maxpt){
  ...
  int imin = specRaw->GetXaxis()->FindBin( minpt + 0.001 );
  int imax = specRaw->GetXaxis()->FindBin( maxpt - 0.001);
...
TH1D * GenSupport::getSPIntnum(double minpt, double maxpt){
  if(!specEffCorrectedCreated) getSpecEffCorrected();
  int imin = specRaw->GetXaxis()->FindBin( minpt + 0.001 );
  int imax = specRaw->GetXaxis()->FindBin( maxpt - 0.001);
  ...
  for(int j = 1; j<= epInt->GetNbinsX(); j++) {
    ...
    double sumerr = 0;
    for(int i =imin; i<=imax; i++) {
      double vn = spvnnum->GetBinContent(i,j);
      double vnerr = spvnnum->GetBinError(i,j);
      double w =specEffCorrected->GetBinContent(i,j);
      double werr = specEffCorrected->GetBinError(i,j);
      if(std::isnormal(vn)&&std::isnormal(vnerr)&&std::isnormal(w)&&std::isnormal(werr)) {
	sum+=w*vn;
    ...
    spIntnum->SetBinContent(j, sum/sumw);
    spIntnum->SetBinError(j,sqrt(sumerr)/sumw);
  }
  return spIntnum;
};
