...
void ReadTree(GetEventInfo * info, string inlist){ 
  ...
  while(fgets(buf,120,flist)!=NULL) {
	...
	qxtrk2_->SetOption("colz");
	qytrk2_->SetOption("colz");
	for(int ievent = 0; ievent<tree->GetEntries(); ievent++) {
	  ...
	  if(ANAL==N2SUB3) Fill_N2SUB3(  bin, qxtrk2_, qytrk2_, qcnt_, Ax, Ay, Bx, By, Cx, Cy, wA, wB, wC);
	  if(ANAL==N42SUB3) Fill_N42SUB3(bin, qxtrk4_, qytrk4_, qcnt_, Ax, Ay, Bx, By, Cx, Cy, wA, wB, wC);
	  ...
	  if(ANAL==N62SUB3) Fill_N62SUB3(bin, qxtrk6_, qytrk6_, qcnt_, qx, qy, sumw);
	  ++NumEvnts;
	}
	tfin->Close();
      }
