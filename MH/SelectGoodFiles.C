...
void SelectGoodFiles(string inlist="testlist.dat") {
  int evcnt=0;
  FILE *  flist;
  flist = fopen(inlist.data(),"r");
  FILE * outlist = fopen("inlist.dat","w");
  char buf[120];
  while(fgets(buf,120,flist)!=NULL) {
    ...
    //std::size_t found = inFile.find("crab_PbPb_2015");
    //std::string outn = inFile.substr(found,string::npos);
    //cout<<outn<<endl; 
    fprintf(outlist,"%s\n",buf);
    tfin->Close();
